...
interface ClubActivitiesSectionProps {
  clubId: string;
  /** Kulüp staff'ı (danışman/officer/başkan) — yönetim butonları görünür */
  canManage: boolean;
}

/** Kulüp detayındaki etkinlik listesi + staff yönetimi (FRONTEND_ETKINLIKLER.md § Kulüp-içi). */
export default function ClubActivitiesSection({ clubId, canManage }: ClubActivitiesSectionProps) {
  ...
  const drafts = canManage ? activities.filter((a) => a.status === "draft") : [];
  const visible = canManage ? activities.filter((a) => a.status !== "draft") : activities;
